async function initializeDiagram() {
    ...
    if (!response.ok) { console.error("Failed to load graph-data.json"); return; }
    const { nodes: nodesData, edges: edgesData } = await response.json();

    const clusterInfo = {
        'planning': { label: 'Preservation Planning' }, 'dissemination': { label: 'Dissemination' },
        'bit-level': { label: 'Bit-level Preservation' }, 'generation': { label: 'Generation of New Files' },
        'other': { label: 'Other Activities' }, 'lifecycle': { label: 'Lifecycle Management' },
    ...
    const edgeMap = edgesData.map(e => ({
        source: nodeMap.get(e.from), target: nodeMap.get(e.to), type: e.type,
    }));
    ...
    for (const id in clusterInfo) {
        ...
        cluster.circleElement = circle; cluster.textElement = text;
    }
    ...
    nodesData.forEach(node => {
        ...
        group.setAttribute('class', 'node-group'); group.id = node.id;
        const circle = document.createElementNS("http://www.w3.org/2000/svg", 'circle');
        ...
        tspan1.setAttribute('x', 0); tspan1.setAttribute('dy', '-0.5em');
        tspan1.textContent = node.label;
        const tspan2 = document.createElementNS("http://www.w3.org/2000/svg", 'tspan');
        tspan2.setAttribute('class', 'node-sublabel');
        tspan2.setAttribute('x', 0); tspan2.setAttribute('dy', '1.2em');
        tspan2.textContent = node.sub;
        textContainer.appendChild(tspan1); textContainer.appendChild(tspan2);
        group.appendChild(circle); group.appendChild(textContainer);
    ...
    function runClusterLayout() {
        ...
        edgeMap.forEach(edge => {
            if (edge.source && edge.target && edge.source.cluster !== edge.target.cluster) {
                connectivity[edge.source.cluster]++;
                connectivity[edge.target.cluster]++;
            }
        });
        const sortedClusters = Object.keys(connectivity).sort((a, b) => connectivity[b] - connectivity[a]);
        const centralClusterId = sortedClusters[0];
        const otherClusterIds = sortedClusters.slice(1);
        clusterInfo[centralClusterId].cx = 0; clusterInfo[centralClusterId].cy = 0;
        ...
        for(let iter = 0; iter < 250; iter++) {
            for(const idA of sortedClusters) {
                for(const idB of sortedClusters) {
                    if (idA >= idB) continue;
                    const cA = clusterInfo[idA]; const cB = clusterInfo[idB];
                    const dx = cA.cx - cB.cx; const dy = cA.cy - cB.cy;
                    ...
                    if (dist < minDist) {
                        ...
                        if (idA !== centralClusterId) { cA.cx += forceX; cA.cy += forceY; }
                        if (idB !== centralClusterId) { cB.cx -= forceX; cB.cy -= forceY; }
                    }
                }
            }
        }
    ...
    function runNodeLayout() {
        for (const id in clusterInfo) {
            ...
            const nodeCircleRadius = Math.max(cluster.radius - nodeRadius - 40, nodeRadius);
    ...
    function updateSVGPositions() {
        ...
        edgeMap.forEach(e => {
            ...
            const sourceSpread = Math.min(maxSpreadAngle, (numSourceEdges - 1) * angleSeparation);
            const targetSpread = Math.min(maxSpreadAngle, (numTargetEdges - 1) * angleSeparation);
            
            const baseSourceAngle = Math.atan2(target.y - source.y, target.x - source.x);
            const baseTargetAngle = Math.atan2(source.y - target.y, source.x - target.x);

            const sourceAngleOffset = (numSourceEdges > 1) ? (sourceEdgeIndex / (numSourceEdges - 1) - 0.5) * sourceSpread : 0;
            const targetAngleOffset = (numTargetEdges > 1) ? (targetEdgeIndex / (numTargetEdges - 1) - 0.5) * targetSpread : 0;
            ...
            if (center_dist < nodeRadius * 2.5) {
                ...
                const perp = { x: -center_dy / center_dist, y: center_dx / center_dist };
                
                // Curve factor increases with distance, making longer links more curved.
                const curveFactor = center_dist * 0.2;
                const control = { x: mid.x + curveFactor * perp.x, y: mid.y + curveFactor * perp.y };

                e.element.setAttribute('d', `M ${p1c.x},${p1c.y} Q ${control.x},${control.y} ${p2c.x},${p2c.y}`);
                // Update the midpoint for the curved line (used for popup positioning).
                e.midPoint = { x: 0.25*p1c.x + 0.5*control.x + 0.25*p2c.x, y: 0.25*p1c.y + 0.5*control.y + 0.25*p2c.y };
            }
        });
    ...
    function centerAndZoom() {
        ...
        if (isFinite(minX)) {
            ...
            const finalScale = Math.min(svg.clientWidth / contentWidth, svg.clientHeight / contentHeight) * 0.95; 
    ...
    function selectNode(node) {
        ...
        const relatedEdges = edgeMap.filter(e => e.source === node || e.target === node);
        relatedEdges.forEach(edge => {
            ...
            linePopup.textContent = relationTypes[edge.type]?.description || relationTypes.default.description;
            linePopupsContainer.appendChild(linePopup);
            linePopup.style.display = 'block'; 
        });

        const nodeNumber = node.id.replace('CPP-', '');
        nodePopup.querySelector('a').href = `https://en.wikipedia.org/${nodeNumber}`;
    ...
    function updatePopupsOnTransform() {
        ...
        const relatedEdges = edgeMap.filter(e => e.source === selectedNode || e.target === selectedNode);
